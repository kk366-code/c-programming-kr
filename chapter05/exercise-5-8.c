...
/* C言語で複数の値を返す慣習的な方法
C言語の関数は、通常、return文を使って1つの値しか返すことができない。
そのため、複数の値を返したい場合は、以下のいずれかの方法を取る（ここでは1の方法を使っている）。

 - 1. ポインタを引数として利用する (最も一般的で推奨される方法)
 - 2. 構造体 (struct) を利用する

返したい値を格納するための変数を、呼び出し元で準備しておき、その変数のアドレス（ポインタ）を関数に渡す。
関数は、受け取ったポインタを使って、そのメモリ位置に結果を直接書き込む。
*/
